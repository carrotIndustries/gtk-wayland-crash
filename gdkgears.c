...
static void
show_popover (GtkButton *button, gpointer data)
{
...
main (int argc, char *argv[])
{
  GtkWidget *window, *box, *hbox, *button, *spinner, *check,
    ...
    *revealer, *frame, *label, *scrolled, *button2, *popover, *label2;
  int i;
  ...
  gtk_container_add(GTK_CONTAINER(popover), label2);
  gtk_widget_show(label2);
  
  button2 = gtk_button_new_with_label("Hallo");
  gtk_container_add (GTK_CONTAINER (box), button2);
  gtk_widget_show(button2);
  gtk_popover_set_relative_to(GTK_POPOVER(popover), button2);
  g_signal_connect (button2, "clicked", G_CALLBACK (show_popover), popover);
